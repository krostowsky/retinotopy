...
for currSubject = 2:length(subjects)
    ...
    % gammaFreqs = find(gammaFreqs == min(gammaFreqs(gammaFreqs > 0))):length(res.freqs);
    % 
    % averageGammaPower = squeeze(nanmean(zscore(log10(res.B(gammaFreqs, :, :)), 0, 2), 1));
    % 
    % %%
    % plotFit = 0;
    % F = res.freqs;
    ...
    % 
    % lowerTheta = [2, 5];
    % avgLowerThetaInd = find(freqs > lowerTheta(1) & freqs < lowerTheta(2));
    % 
    % upperTheta = [5, 8];
    % avgUpperThetaInd = find(freqs > upperTheta(1) & freqs < upperTheta(2));
    % 
    % powerMat = zeros(numEvents, 3, length(resultLabel)); % columns are lower theta power, upper theta power, gamma power
    % 
    % %%
    % eventCount = 0;
    % for j = 1:length(onEventIndices)
    %     currBlockEventIndices = onEventIndices{j};
    %     for jj = 1:size(currBlockEventIndices,2)
    %         eventCount = eventCount + 1;
    % 
    %         currEventInds = currBlockEventIndices(:,jj);
    %         currEventInds = currEventInds(currEventInds ~= 0); % this is to remove any 0 padding added by buffer
    % 
    %         % compute power spectrum for each electrode at the given event &
    %         % perform ap fit
    %         currData = res.B(:,currEventInds,:);
    % 
    %         currAvgLowerTheta = squeeze(log10(nanmean(nanmean(currData(avgLowerThetaInd,:,:),1),2)));
    %         currAvgUpperTheta = squeeze(log10(nanmean(nanmean(currData(avgUpperThetaInd,:,:),1),2)));
    %         currAvgGamma = nanmean(averageGammaPower(currEventInds, :), 1);
    % 
    %         powerMat(eventCount, :, :) = [currAvgLowerTheta, currAvgUpperTheta, currAvgGamma']';
    % 
    %         % this performs the model fits for each electrode at the given
    %         % event
    %         if plotFit
    %             ps = squeeze(log10(nanmean(currData,2)));
    %             for k = 1:size(currData, 3)
    % 
    %                 currPs = ps(:,k);
    %                 ap_guess = [nan, currPs(1), 0];
    % 
    %                 [ap_params, ap_ps] = robust_ap_fit(freqs(1:upperBoundInd), currPs(1:upperBoundInd)', ap_guess);
    % 
    %                 fitFig = figure; plot(F(1:length(ap_ps)), ap_ps, 'LineStyle', '-'); hold on; plot(F, ps(:,k), 'LineStyle', '--');
    %                 legend('fitted model of AP power',' full power spectrum');
    % 
    %                 saveas(fitFig, [figDir '/' subjId '/events/' resultLabel{k} '-block-' num2str(j) '-event-' num2str(jj) '.png']);
    %                 close();
    %             end
    %         end
    %     end
    ...
    % 
    % %
    % for j = 1:length(onEventIndicesOff)
    %     currBlockEventIndices = onEventIndicesOff{j};
    %     for jj = 1:size(currBlockEventIndices,2)
    %         eventCount = eventCount + 1;
    % 
    %         currEventInds = currBlockEventIndices(:,jj);
    ...
    % 
    % %
    % for j = 1:length(offEventIndices)
    %     currBlockEventIndices = offEventIndices{j};
    %     for jj = 1:size(currBlockEventIndices,2)
    %         eventCount = eventCount +1;
    % 
    %         currEventInds = currBlockEventIndices(:,jj);
    ...
    %                 legend('fitted model of AP power',' full power spectrum');
    % 
    %                 saveas(fitFig, [figDir '/' subjId '/events/' resultLabel{k} '-block-' num2str(j) '-eventOff-' num2str(jj) '.png']);
    %                 close();
    %             end
    %         end
    %     end
    ...
    if makeEvents
        ...
        %saveMe = load([outdir '/' subjects{currSubject} '/events.mat'], 'onEventImages');
        %onEventImages = saveMe.onEventImages;
        %clearvars saveMe
        %save([outdir '/' subjects{currSubject} '/eventsNew.mat']);
    end
    toc;
end
