function [fixedTTLs, behavAdjust] = makeNewTTL(indir, subjid, behavFiles, Timestamps, exampleSamples)
...
[~, minInd] = min(abs(exampleSamples - Timestamps(1))); % gives time closest to end TTL
checkNeg = (exampleSamples - Timestamps(1));
checkNeg = checkNeg(minInd);
if checkNeg < 0
    endInd = minInd;
else
    endInd = minInd - 1;
...
timePerSample = abs(exampleSamples(1) - exampleSamples(2)) / 512; % 512 comes from the neuralynx record size
behavAdjust = (timePerSample * endInd) + correctionFactor;

% to use the behavAdjust, subtract it from the first
% recorded TTL time, any times from the behavior before then should be
% removed

%findFirstSampleTime = Timestamps(1) - (collectionTimesStimuli(1) + correctionFactor);
findFirstSampleTime = Timestamps(1) - (behavAdjust);
